...
async function seed() {
  ...
  const insComm = db.prepare('INSERT INTO communities (name) VALUES (?)')
  comms.forEach(c => insComm.run(c))
  ...
  Object.entries(settings).forEach(([k,v]) => insSetting.run(k, JSON.stringify(v)))

  // Default resources
  const insRes = db.prepare(`INSERT INTO resources (title,description,resource_type,external_url,category,tags,created_by) VALUES (?,?,?,?,?,?,?)`)
  const resources = [
  ...
  resources.forEach(r => insRes.run(...r))

  // 5 Pre-made quizzes with full questions
  seedQuizzes()

  // Sample monitoring sites in Northern Ontario
  const insSite = db.prepare(`INSERT INTO sites (name,latitude,longitude,body_of_water,water_body_type,access_risk,community,parameters_tested,created_by) VALUES (?,?,?,?,?,?,?,?,?)`)
  const params = JSON.stringify(['ph','dissolved_oxygen','turbidity','conductivity','water_temp'])
  insSite.run('Garden River Site A',46.4833,-84.0833,'Garden River','River','Low','Sault Ste. Marie',params,1)
  insSite.run('Lake Huron Shore Station',46.5200,-84.3500,'Lake Huron','Lake','Low','Sault Ste. Marie',params,1)
  insSite.run('Mississagi River Monitor',46.0167,-83.0500,'Mississagi River','River','Medium','Blind River',params,1)
  insSite.run('Thessalon River Site',46.2500,-83.5500,'Thessalon River','River','Low','Thessalon',params,1)
  insSite.run('Chapleau Crown Game Preserve',47.8333,-83.4000,'Chapleau River','River','Medium','Chapleau',params,1)

  // AI Knowledge base
  seedKnowledgeBase()

  // Default sponsor
  db.prepare(`INSERT INTO sponsors (name,website_url,tagline,tier,placement,status) VALUES (?,?,?,?,?,?)`).run(
...
function seedQuizzes() {
  const insQ = db.prepare(`INSERT INTO quizzes (title,description,category,difficulty,time_per_question,pass_score,status,created_by) VALUES (?,?,?,?,?,?,?,?)`)
  const insQQ = db.prepare(`INSERT INTO quiz_questions (quiz_id,question_type,question_text,options,correct_answers,explanation,points,sort_order) VALUES (?,?,?,?,?,?,?,?)`)

  // Quiz 1: Water Quality Fundamentals
  const q1 = insQ.run('Water Quality Fundamentals','Test your knowledge of essential water quality parameters and concepts.','Water Quality','Beginner',60,70,'published',1)
  const quiz1Qs = [
  ...
  quiz1Qs.forEach(([type,text,opts,correct,expl,pts],i) => insQQ.run(q1.lastInsertRowid,type,text,opts,correct,expl,pts||1,i))

  // Quiz 2: Field Sampling Procedures
  const q2 = insQ.run('Field Sampling Procedures','Learn proper water sampling protocols for reliable results.','Field Work','Intermediate',60,70,'published',1)
  const quiz2Qs = [
  ...
  quiz2Qs.forEach(([type,text,opts,correct,expl,pts],i) => insQQ.run(q2.lastInsertRowid,type,text,opts,correct,expl,pts||1,i))

  // Quiz 3: Reading Water Reports
  const q3 = insQ.run('Reading Water Reports','Understand how to interpret water quality reports and data.','Data Literacy','Intermediate',90,70,'published',1)
  const quiz3Qs = [
  ...
  quiz3Qs.forEach(([type,text,opts,correct,expl,pts],i) => insQQ.run(q3.lastInsertRowid,type,text,opts,correct,expl,pts||1,i))

  // Quiz 4: Aquatic Ecosystems
  const q4 = insQ.run('Aquatic Ecosystems','Explore the living world of Northern Ontario\'s lakes and rivers.','Ecology','Beginner',60,70,'published',1)
  const quiz4Qs = [
  ...
  quiz4Qs.forEach(([type,text,opts,correct,expl,pts],i) => insQQ.run(q4.lastInsertRowid,type,text,opts,correct,expl,pts||1,i))

  // Quiz 5: Northern Ontario Water
  const q5 = insQ.run('Northern Ontario Water Challenges','Advanced topics in Northern Ontario water quality and stewardship.','Regional','Advanced',90,70,'published',1)
  const quiz5Qs = [
  ...
  quiz5Qs.forEach(([type,text,opts,correct,expl,pts],i) => insQQ.run(q5.lastInsertRowid,type,text,opts,correct,expl,pts||1,i))
}

function seedKnowledgeBase() {
  // Knowledge base stored in ai_cache as pre-seeded Q&A
  const kb = [
  ...
  const ins = db.prepare('INSERT OR IGNORE INTO ai_cache (question_hash,question,answer) VALUES (?,?,?)')
