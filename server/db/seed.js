...
async function seed() {
  ...
  if (!adminExists) {
    ...
    await db.run(
      `INSERT INTO users (username,email,password_hash,display_name,role,avatar_emoji,avatar_bg_color,is_admin,onboarding_completed)
       VALUES (?,?,?,?,?,?,?,?,?) ON CONFLICT DO NOTHING`,
      ['admin','info@nordikinstitute.com',hash,'SOURCE Water Admin','SOURCE Water team member','🌊','#0ea5e9',1,1])
