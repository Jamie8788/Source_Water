...
// ── GLB Mascot Model ──────────────────────────────────────────────────────────
function GLBMascotModel() {
  const groupRef = useRef(null)
  console.time('glb-load-time')
  const { scene } = useGLTF('/models/Water_Mascot.glb')
  console.timeEnd('glb-load-time')

  useFrame(({ clock }) => {
    if (!groupRef.current) return
    const t = clock.elapsedTime
    const breatheScale = 1 + Math.sin(t * 1.5) * 0.03
...
// ── Main WaterMascotGLB ───────────────────────────────────────────────────────
export default function WaterMascotGLB() {
        ...
        <div className="flex items-end gap-2">
          ...
          <div
            ...
            style={{
              ...
              transform: bounce ? `scale(1.2) translateY(-12px)` : 'scale(1)',
            ...
            <Canvas
              ...
              <Suspense fallback={null}>
                ...
                <directionalLight position={[-2.5, 3, 2]} intensity={1.4} castShadow shadow-mapSize={[512, 512]}/>
                <directionalLight position={[3, 1, -1]} intensity={0.4} color="#b8d4f0"/>
                <pointLight position={[0, -1, 2]} intensity={0.3} color="#ffa4c0"/>
                <Environment preset="studio"/>
                <Float speed={2} rotationIntensity={0.08} floatIntensity={0.4}>
                  <GLBMascotModel />
                </Float>
                <ContactShadows position={[0,-0.9,0]} opacity={0.3} scale={2.5} blur={1.8} far={2.5}/>
              </Suspense>
            </Canvas>
          </div>
        </div>
